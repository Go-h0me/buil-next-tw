...
useEffect(() =>onSnapshot(query(
  ...
  return (
    <div
      className="flex cursor-pointer border-b border-gray-700
    p-3 
    "
        ...
        <div className={`flex ${!postPage && 'justify-between'}`}>
          ...
          <div className="text-[#6e767d]">
            <div className="group inline-block">
              <h4
                className={`text-[15px] font-bold text-[#d9d9d9]
            group-hover:underline sm:text-base ${!postPage && 'inline-block'}
            `}
              >
                {post?.username}
              </h4>
              <span
                className={`sm:text-[15px ] text-sm ${!postPage && 'ml-1.5'}`}
              >
                @{post?.tag}
              </span>
            </div>{""}
